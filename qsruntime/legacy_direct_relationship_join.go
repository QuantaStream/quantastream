...
func (e LegacyDirectRelationshipVectorJoinExecutor) executeLegacyDirectRelationshipVectorJoinGraph(ctx context.Context, request ExecutionRequest, vector RelationshipVectorJoinRequest) (ExecutionResult, error) {
	...
	request = legacyDirectRelationshipRequestWithoutAppliedResidualPrefilters(request, appliedPrefilterPredicates)
	equalitySeedMode := legacyDirectRelationshipGraphEqualityRoleSeedModeFromEnv()
	equalitySeedFields := legacyDirectRelationshipGraphEqualityFields(request)
	equalitySeedOptions := legacyDirectRelationshipGraphEqualityRoleSeedOptions{}
	if equalitySeedMode == legacyDirectRelationshipGraphEqualityRoleSeedModeOff {
		equalitySeedFields = nil
	} else if equalitySeedMode == legacyDirectRelationshipGraphEqualityRoleSeedModeAuto {
		equalitySeedOptions.maxSourceRows = legacyDirectRelationshipGraphEqualityRoleSeedAutoMaxSourceRows
	}
	...
	result := ExecutionResult{Probes: []ExecutionProbe{
		...
		legacyDirectRelationshipProbe("graph_equality_role_seed_enabled", strconv.FormatBool(len(equalitySeedFields) > 0)),
		legacyDirectRelationshipProbe("graph_equality_role_seed_mode", string(equalitySeedMode)),
		legacyDirectRelationshipProbe("graph_equality_role_seed_auto_max_source_rows", strconv.Itoa(equalitySeedOptions.maxSourceRows)),
	...
	for {
		...
		if len(equalitySeedFields) > 0 {
			equalitySeedProbes, equalitySeedChanged, diagnostics, err := e.legacyDirectRelationshipApplyGraphEqualityRoleSeedsForFieldsWithOptions(ctx, request, equalitySeedFields, rowsByTable, fullDomainInitialRowsByRole, fmt.Sprintf("graph_iter_%d_pre_", iterations), equalitySeedOptions)
		...
		for edgeIndex, candidate := range executionCandidates {
			...
			if len(nextChildRows) != len(childRows) {
				...
				if len(equalitySeedFields) > 0 {
					equalitySeedProbes, equalitySeedChanged, diagnostics, err := e.legacyDirectRelationshipApplyGraphEqualityRoleSeedsForFieldsWithOptions(ctx, request, equalitySeedFields, rowsByTable, fullDomainInitialRowsByRole, probePrefix+"post_", equalitySeedOptions)
					result.Probes = append(result.Probes, equalitySeedProbes...)
					result.Diagnostics = append(result.Diagnostics, diagnostics...)
					if err != nil || result.Diagnostics.BlocksNative() {
						return result, err
...
func legacyDirectRelationshipSortedRownumsCopy(rownums []qsbridge.QuantaRownum) []qsbridge.QuantaRownum {
	sorted := append([]qsbridge.QuantaRownum(nil), rownums...)
	if legacyDirectRelationshipRownumsAscending(sorted) {
		return sorted
	}
...
func (e LegacyDirectRelationshipVectorJoinExecutor) legacyDirectRelationshipReverseArtifactChildRows(ctx context.Context, edge legacyDirectRelationshipEdge, childRows []qsbridge.QuantaRownum, parentKeyRows map[int64]qsbridge.QuantaRownum, options legacyDirectRelationshipReduceOptions) ([]qsbridge.QuantaRownum, qsbridge.FilterDomainRelationshipVectorResult, map[qsbridge.QuantaRownum]qsbridge.QuantaRownum, []legacyDirectRelationshipPair, legacyDirectRelationshipReverseArtifactLocalTiming, bool, qsbridge.DiagnosticSet, error) {
	...
	start := time.Now()
	artifactRead, artifactTiming, diagnostics, err, ok := backend.readRelationshipVectorReverseArtifactCandidates(ctx, projectionKey, read, sourceValues)
	elapsed := time.Since(start)
	candidates := artifactRead.Candidates
	...
	if options.omitReverseArtifactParentValues {
		if localTiming.targetCandidateMode == "omitted_full_domain" {
			narrowedRows := legacyDirectRelationshipSortedRownumsCopy(candidates.Rownums)
			localTiming.narrowElapsed = time.Since(narrowStart)
			localTiming.mode = "row_filter_only_sorted_candidates"
...
func legacyDirectRelationshipReverseArtifactRowFilterOnlyMode(mode string) bool {
	return mode == "row_filter_only" || mode == "row_filter_only_sorted_candidates"
}
...
func legacyDirectRelationshipRowsFromRawArtifactCandidateRows(candidateRows []qsbridge.QuantaRownum, rawParentValueByChild map[uint64]int64, parentValueByChild map[qsbridge.QuantaRownum]int64, parentKeyRows map[int64]qsbridge.QuantaRownum) ([]qsbridge.QuantaRownum, map[qsbridge.QuantaRownum]qsbridge.QuantaRownum, []legacyDirectRelationshipPair, bool) {
	if len(rawParentValueByChild) == 0 {
		return legacyDirectRelationshipRowsFromArtifactCandidateRows(candidateRows, parentValueByChild, parentKeyRows)
	}
	if len(candidateRows) != len(rawParentValueByChild) {
		return legacyDirectRelationshipRowsFromRawArtifactParentValueMap(rawParentValueByChild, parentKeyRows)
	}
	narrowedRows := make([]qsbridge.QuantaRownum, 0, len(rawParentValueByChild))
	parentByChild := make(map[qsbridge.QuantaRownum]qsbridge.QuantaRownum, len(rawParentValueByChild))
	pairs := make([]legacyDirectRelationshipPair, 0, len(rawParentValueByChild))
	for _, child := range candidateRows {
		parentValue, ok := rawParentValueByChild[uint64(child)]
		if !ok {
			return nil, nil, nil, false
		}
		parentRow, ok := parentKeyRows[parentValue]
...
func legacyDirectRelationshipRowsFromRawArtifactParentValueMap(rawParentValueByChild map[uint64]int64, parentKeyRows map[int64]qsbridge.QuantaRownum) ([]qsbridge.QuantaRownum, map[qsbridge.QuantaRownum]qsbridge.QuantaRownum, []legacyDirectRelationshipPair, bool) {
	if len(rawParentValueByChild) == 0 || len(parentKeyRows) == 0 {
		return nil, nil, nil, false
	}
	narrowedRows := make([]qsbridge.QuantaRownum, 0, len(rawParentValueByChild))
	parentByChild := make(map[qsbridge.QuantaRownum]qsbridge.QuantaRownum, len(rawParentValueByChild))
	pairs := make([]legacyDirectRelationshipPair, 0, len(rawParentValueByChild))
	for child, parentValue := range rawParentValueByChild {
		parentRow, ok := parentKeyRows[parentValue]
		if !ok {
			return nil, nil, nil, false
		}
		childRow := qsbridge.QuantaRownum(child)
		narrowedRows = append(narrowedRows, childRow)
		parentByChild[childRow] = parentRow
		pairs = append(pairs, legacyDirectRelationshipPair{child: childRow, parent: parentRow})
	}
	return narrowedRows, parentByChild, pairs, true
}
