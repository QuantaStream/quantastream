...
// BindShowPlugins binds parser-neutral SHOW PLUGINS metadata into QueryIR.
func BindShowPlugins(context *BindContext, showStmt UnboundShowPlugins) (QueryIR, DiagnosticSet) {
	_ = context
	return QueryIR{
...
func bindMembershipPredicateContext(context *BindContext, membership UnboundMembership, right FieldRef) (*BindContext, DiagnosticSet) {
	...
	if membership.RightTable.InlineRows != nil {
		bound, _, diagnostics := context.bindInlineRowSetTable(membership.RightTable, right.Table.ID)
		if diagnostics.BlocksNative() {
			return nil, diagnostics
		}
