...
func (c *BitmapIndex) query(query *pb.BitmapQuery, disableFragmentFanout ...bool) (*roaring64.Bitmap, uint64, []QueryResultProbe, error) {
	...
	for k, v := range indexGroups {
		...
		eg.Go(func() error {
			...
			ir.probes = append(ir.probes, groupProbes...)
			resultChan <- ir
			return nil
		})
	}
	...
	for rs := range resultChan {
		probes = append(probes, rs.probes...)
	...
	for _, v := range resultsMap {
		...
		if v.FKCount() == 0 {
			if driver != "" {
				return nil, 0, nil, fmt.Errorf("Must specify at least 1 join for index %s", v.Index)
			}
			driver = v.Index
		}
	...
	for _, v := range resultsMap {
		for _, fk := range v.GetFKList() {
			...
			if !ok {
				return nil, 0, nil, fmt.Errorf("Can't find FK %s", fk.JoinIndex)
			}
	...
	return result, count, probes, nil
}
...
func (c *BitmapIndex) queryGroup(index string, query *pb.BitmapQuery, disableFragmentFanout bool) (*IntermediateResult, []QueryResultProbe, error) {
	...
	if c.local != nil {
		...
		probes := queryResultProbes(result)
...
func (c *BitmapIndex) queryGroupWholeFanout(index string, query *pb.BitmapQuery) (*IntermediateResult, []QueryResultProbe, error) {
	...
	probes := make([]QueryResultProbe, 0)
	for rs := range resultChan {
		probes = append(probes, queryResultProbes(rs)...)
		ir := NewIntermediateResult(index)
...
func queryResultProbes(result *pb.QueryResult) []QueryResultProbe {
	...
	for _, sample := range result.GetSamples() {
		if probe, ok := QueryResultProbeFromSample(sample); ok {
...
func (c *BitmapIndex) Join(driverIndex string, fklist []string, fromTime, toTime int64,
	...
	// Send the same join request to each currently connected node.
	for i, n := range c.clientsSnapshot() {
		client := n
		clientIndex := i
		eg.Go(func() error {
