...
func (m *Conn) SelectNodes(key interface{}, op OpType) ([]int, error) {
	...
	return m.selectNodesLocked(key, op)
}

func (m *Conn) selectNodesLocked(key interface{}, op OpType) ([]int, error) {
...
// StopPolling signals the membership poll loop to exit while leaving client
// connections open for any final in-flight work.
func (m *Conn) StopPolling() {
	select {
	case _, open := <-m.Stop:
		if open {
			close(m.Stop)
