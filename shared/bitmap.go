...
func (c *BitmapIndex) RelationshipReverseArtifactCandidateValuesForRowsUnordered(index, field string, sourceValues []int64, candidateRows []uint64) ([]uint64, map[uint64]int64, RelationshipReverseArtifactStats, bool, error) {
	rownums, _, parentValueByChild, stats, ok, err := c.relationshipReverseArtifactCandidateValuesForRows(index, field, sourceValues, candidateRows, false, false, false, true)
...
func (c *BitmapIndex) RelationshipReverseArtifactCandidateValuesForRowsUnorderedDerived(index, field string, sourceValues []int64, candidateRows []uint64) ([]uint64, map[uint64]int64, RelationshipReverseArtifactStats, bool, error) {
	rownums, _, parentValueByChild, stats, ok, err := c.relationshipReverseArtifactCandidateValuesForRows(index, field, sourceValues, candidateRows, false, true, false, true)
	return rownums, parentValueByChild, stats, ok, err
}
...
func (c *BitmapIndex) RelationshipReverseArtifactCandidateRowsForRowsUnordered(index, field string, sourceValues []int64, candidateRows []uint64) ([]uint64, RelationshipReverseArtifactStats, bool, error) {
	rownums, _, _, stats, ok, err := c.relationshipReverseArtifactCandidateValuesForRows(index, field, sourceValues, candidateRows, false, false, true, false)
...
func (c *BitmapIndex) RelationshipReverseArtifactCandidateValuesForRows(index, field string, sourceValues []int64, candidateRows []uint64) ([]uint64, map[uint64]int64, RelationshipReverseArtifactStats, bool, error) {
	rownums, _, parentValueByChild, stats, ok, err := c.relationshipReverseArtifactCandidateValuesForRows(index, field, sourceValues, candidateRows, true, true, false, true)
	return rownums, parentValueByChild, stats, ok, err
}

func (c *BitmapIndex) relationshipReverseArtifactCandidateValuesForRows(index, field string, sourceValues []int64, candidateRows []uint64, sortRows bool, deriveRowsFromParentValues bool, omitParentValues bool, collectParentMap bool) ([]uint64, []int64, map[uint64]int64, RelationshipReverseArtifactStats, bool, error) {
	if index == "" {
		return nil, nil, nil, RelationshipReverseArtifactStats{}, false, fmt.Errorf("RelationshipReverseArtifactCandidateValues: index not specified")
	}
	if field == "" {
		return nil, nil, nil, RelationshipReverseArtifactStats{}, false, fmt.Errorf("RelationshipReverseArtifactCandidateValues: field not specified")
	}
	...
	if c.local != nil {
		...
		if err != nil {
			return nil, nil, nil, RelationshipReverseArtifactStats{}, false, err
		}
		mergeStart := time.Now()
		rownums, parentValues, parentValueByChild, stats, ok, err := aggregateRelationshipReverseArtifactCandidateResponses([]*pb.RelationshipReverseArtifactCandidatesResponse{response}, sourceValues, sortRows, deriveRowsFromParentValues, collectParentMap)
		...
		return rownums, parentValues, parentValueByChild, stats, ok, err
	}

	clients := c.activeClientsSnapshot()
	if len(clients) == 0 {
		return nil, nil, nil, RelationshipReverseArtifactStats{}, false, fmt.Errorf("RelationshipReverseArtifactCandidateValues: no active bitmap nodes")
	}
	...
	mergeStart := time.Now()
	rownums, parentValues, parentValueByChild, stats, ok, err := aggregateRelationshipReverseArtifactCandidateResponses(responses, sourceValues, sortRows, deriveRowsFromParentValues, collectParentMap)
...
func (c *BitmapIndex) RelationshipReverseArtifactStats(index, field string) (RelationshipReverseArtifactStats, bool, error) {
	...
	if c.local != nil {
		...
		stats, ok, err := aggregateRelationshipReverseArtifactStatsResponses([]relationshipReverseArtifactStatsClientResult{{
			response: response,
			elapsed:  callElapsed,
		}})
		stats.FanoutElapsed = callElapsed
	...
	resultChan := make(chan relationshipReverseArtifactStatsClientResult, len(clients))
	var eg errgroup.Group
	fanoutStart := time.Now()
	for _, n := range clients {
		client := n.client
		clientIndex := n.index
		eg.Go(func() error {
			response, elapsed, err := c.relationshipReverseArtifactStatsClient(client, req, clientIndex)
			if err != nil {
				return err
			}
			resultChan <- relationshipReverseArtifactStatsClientResult{
				response: response,
				elapsed:  elapsed,
			}
			return nil
	...
	responses := make([]relationshipReverseArtifactStatsClientResult, 0, len(clients))
	for result := range resultChan {
		responses = append(responses, result)
	}
	stats, ok, err := aggregateRelationshipReverseArtifactStatsResponses(responses)
	stats.FanoutElapsed = fanoutElapsed
	return stats, ok, err
}
...
// RelationshipVectorValueSum asks readable nodes to expand source-domain values
// through maintained reverse relationship artifacts and aggregate child-domain
// BSI values grouped by source value.
func (c *BitmapIndex) RelationshipVectorValueSum(index, vectorField, valueField string, fromTime, toTime int64, sourceValues []int64) ([]RelationshipAlignedValueSumGroup, RelationshipAlignedValueSumStats, bool, error) {
	if index == "" {
		return nil, RelationshipAlignedValueSumStats{}, false, fmt.Errorf("RelationshipVectorValueSum: index not specified")
	}
	if vectorField == "" {
		return nil, RelationshipAlignedValueSumStats{}, false, fmt.Errorf("RelationshipVectorValueSum: vector field not specified")
	}
	if valueField == "" {
		return nil, RelationshipAlignedValueSumStats{}, false, fmt.Errorf("RelationshipVectorValueSum: value field not specified")
	}
	req := &pb.RelationshipVectorValueSumRequest{
		Index:        index,
	...
	if c.local != nil {
		...
		defer cancel()
		callStart := time.Now()
		response, err := c.local.RelationshipVectorValueSum(ctx, req)
		callElapsed := time.Since(callStart)
		if err != nil {
			return nil, RelationshipAlignedValueSumStats{}, false, err
		}
		groups, stats, ok, err := aggregateRelationshipVectorValueSumResponses([]relationshipVectorValueSumClientResult{{
			response: response,
			elapsed:  callElapsed,
		}}, sourceValues)
		stats.ClientRPCElapsed = callElapsed
		stats.MaxClientRPCElapsed = callElapsed
		return groups, stats, ok, err
	}

	clients := c.activeClientsSnapshot()
	if len(clients) == 0 {
		return nil, RelationshipAlignedValueSumStats{}, false, fmt.Errorf("RelationshipVectorValueSum: no active bitmap nodes")
	}
	resultChan := make(chan relationshipVectorValueSumClientResult, len(clients))
	var eg errgroup.Group
	for _, n := range clients {
		client := n.client
		clientIndex := n.index
		eg.Go(func() error {
			response, elapsed, err := c.relationshipVectorValueSumClient(client, req, clientIndex)
			if err != nil {
				return err
			}
			resultChan <- relationshipVectorValueSumClientResult{
				response: response,
				elapsed:  elapsed,
			}
			return nil
	...
	responses := make([]relationshipVectorValueSumClientResult, 0, len(clients))
	for result := range resultChan {
		responses = append(responses, result)
	}
	return aggregateRelationshipVectorValueSumResponses(responses, sourceValues)
}
...
func aggregateRelationshipReverseArtifactCandidateResponses(responses []*pb.RelationshipReverseArtifactCandidatesResponse, sourceValues []int64, sortRows bool, deriveRowsFromParentValues bool, collectParentMap bool) ([]uint64, []int64, map[uint64]int64, RelationshipReverseArtifactStats, bool, error) {
	...
	var parentValueByChild map[uint64]int64
	if collectParentMap {
	...
	if sortRows {
		...
		if len(parentValues) == len(rownums) {
			...
			pairs := make([]rowParentValue, len(rownums))
			for i, rownum := range rownums {
				pairs[i] = rowParentValue{rownum: rownum, parentValue: parentValues[i]}
			}
			sort.Slice(pairs, func(i, j int) bool { return pairs[i].rownum < pairs[j].rownum })
			for i, pair := range pairs {
				rownums[i] = pair.rownum
				parentValues[i] = pair.parentValue
			}
		} else {
...
type relationshipReverseArtifactStatsClientResult struct {
	response *pb.RelationshipReverseArtifactStatsResponse
	elapsed  time.Duration
}

func aggregateRelationshipReverseArtifactStatsResponses(responses []relationshipReverseArtifactStatsClientResult) (RelationshipReverseArtifactStats, bool, error) {
...
type relationshipVectorValueSumClientResult struct {
	response *pb.RelationshipVectorValueSumResponse
	elapsed  time.Duration
}
...
func aggregateRelationshipVectorValueSumResponses(responses []relationshipVectorValueSumClientResult, sourceValues []int64) ([]RelationshipAlignedValueSumGroup, RelationshipAlignedValueSumStats, bool, error) {
	stats := RelationshipAlignedValueSumStats{
		SourceValues: relationshipReverseArtifactUniqueInt64Count(sourceValues),
	}
	groupsByParent := make(map[uint64]*RelationshipAlignedValueSumGroup)
	...
	for _, result := range responses {
		...
		for _, protoGroup := range response.GetGroups() {
			...
			if protoGroup.GetSum() == "" {
				...
				return nil, RelationshipAlignedValueSumStats{}, false, fmt.Errorf("RelationshipVectorValueSum: could not parse sum %q", protoGroup.GetSum())
			}
			parent := protoGroup.GetParentValue()
			group := groupsByParent[parent]
			if group == nil {
...
func (c *BitmapIndex) relationshipReverseArtifactStatsClient(client pb.BitmapIndexClient, req *pb.RelationshipReverseArtifactStatsRequest,
	clientIndex int) (*pb.RelationshipReverseArtifactStatsResponse, time.Duration, error) {
...
func (c *BitmapIndex) relationshipVectorValueSumClient(client pb.BitmapIndexClient, req *pb.RelationshipVectorValueSumRequest,
	clientIndex int) (*pb.RelationshipVectorValueSumResponse, time.Duration, error) {

	ctx, cancel := context.WithTimeout(context.Background(), Deadline)
	defer cancel()

	start := time.Now()
	result, err := client.RelationshipVectorValueSum(ctx, req)
	elapsed := time.Since(start)
	if err != nil {
