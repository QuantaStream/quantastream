...
func TestValidateFlagsAllowsInaboxStandardWithoutConnectionFlags(t *testing.T) {
	cfg := runnerConfig{Engine: engineInaboxStandard}

	if err := validateFlags("sqltests/basic_queries.yaml", cfg); err != nil {
		t.Fatalf("inabox-standard validation should default local server connection flags: %v", err)
	}
}
